...
static ImGui_ImplOpenGL3_Data *ImGui_ImplOpenGL3_GetBackendData()
{
	return ImGui::GetCurrentContext()
			   ? static_cast<ImGui_ImplOpenGL3_Data*>(ImGui::GetIO().BackendRendererUserData)
			   : nullptr;
}
...
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData *draw_data)
{
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		const GLsizeiptr vtx_buffer_size = static_cast<GLsizeiptr>(cmd_list->VtxBuffer.Size) * static_cast<int>(sizeof(
			ImDrawVert));
		const GLsizeiptr idx_buffer_size = static_cast<GLsizeiptr>(cmd_list->IdxBuffer.Size) * static_cast<int>(sizeof(
			ImDrawIdx));
		if (bd->UseBufferSubData)
		{
			...
			GL_CALL(
				glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, idx_buffer_size, static_cast<const GLvoid*>(cmd_list->
					IdxBuffer.Data)));
		}
		else
		{
			GL_CALL(
				glBufferData(GL_ARRAY_BUFFER, vtx_buffer_size, static_cast<const GLvoid*>(cmd_list->VtxBuffer.Data),
					GL_STREAM_DRAW));
			GL_CALL(
				glBufferData(GL_ELEMENT_ARRAY_BUFFER, idx_buffer_size, static_cast<const GLvoid*>(cmd_list->IdxBuffer.
					Data), GL_STREAM_DRAW));
		}
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
		{
			...
			else
			{
				...
				if (bd->GlVersion >= 320)
					GL_CALL(
					glDrawElementsBaseVertex(GL_TRIANGLES, static_cast<GLsizei>(pcmd->ElemCount), sizeof(ImDrawIdx) == 2
						? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, (void*)static_cast<intptr_t>(pcmd->IdxOffset * sizeof(
